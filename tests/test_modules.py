...
class TestLayers:
    ...
    def test_linear_no_bias(self):
        ...
        assert (
            "b" not in layer["params"]
        ), "Bias parameter should not exist when has_bias=False"
    ...
    def test_linear_input_validation(self):
        ...
        with pytest.raises(ValueError):
            layer["fn"](
                nw.randn([self.batch_size, self.in_features + 1])
            )  # Wrong feature dim
        with pytest.raises(ValueError):
...
class TestSequential:
    """Tests for the main Sequential class functionality."""

    def setup_method(self):
        """Initialize a new Sequential for each test."""
        self.net = nw.Sequential()
    ...
    def test_forward_pass(self):
        ...
        assert (
            output.shape == expected_shape
        ), f"Expected final output shape {expected_shape}, got {output.shape}"
