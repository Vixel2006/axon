...
__global__ void neg_kernel(const float* __restrict__ a_data, float* __restrict__ c_data, size_t num_elements) {
...
Tensor CudaOps::neg(const Tensor &a) {
    if (a.device().type != DeviceType::CUDA) {
        throw std::runtime_error("Input tensors for CudaOps::neg must be on the CUDA device.");
    }
    ...
    neg_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_c, num_elements);
