...
/**
 * @brief CUDA kernel for element-wise natural logarithm of a tensor.
 *
 * This kernel uses a grid-stride loop to ensure all elements are processed. It applies
 * the single-precision natural logarithm function, logf(), to each element. Note that
 * the behavior for non-positive inputs follows the standard for logf (NaN for < 0, -inf for 0).
 *
...
__global__ void log_kernel(const float* a_data, float* c_data, size_t num_elements) {
    ...
    for (int i = index; i < num_elements; i += stride) {
        c_data[i] = logf(a_data[i]); // Use logf for single-precision floats
    }
}
...
Tensor CudaOps::log(const Tensor &a) {
    ...
    if (a.device().type != DeviceType::CUDA) {
        throw std::runtime_error("Input tensor for CudaOps::log must be on the CUDA device.");
    }
    ...
    log_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_a, d_c, num_elements);
