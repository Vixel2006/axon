    ...
    if (n_prev == 2) {
        ...
        if (!is_contiguous(a) || !is_contiguous(out)) {
            ...
            if (a->requires_grad) {
                ...
                for (; i < size; ++i) {
                    a->grad->data[i] += out->grad->data[i] * b;
                }
            }
        }
    }
...
void pow_grad_op(Tensor* out, Tensor** prev, int n_prev, void* extras) {
    ...
    if (n_prev != 1 && n_prev != 2) {
        LOG_ERROR("pow_grad_op ERROR: Invalid number of previous tensors: %d. "
                  "Expected 1 or 2.",
                  n_prev);
        return;
    }
    ...
    if (n_prev == 1) { // Tensor ** scalar
        ...
        if (!a->requires_grad && !b_tensor->requires_grad)
            return;

        if (!is_contiguous(a) || !is_contiguous(b_tensor) || !is_contiguous(out)) {
            // Fallback for non-contiguous tensors (implement if needed)
            // For now, we'll use a basic loop
            for (int linear = 0; linear < size; ++linear) {
                int idx = linear;
                int a_offset = 0, b_offset = 0, out_offset = 0;
                for (int d = out->ndim - 1; d >= 0; --d) {
                    ...
                    b_offset += coord * b_tensor->strides[d];
                    out_offset += coord * out->strides[d];
                }

                float dout = out->grad->data[out_offset];
                float aval = a->data->data[a_offset];
                float bval = b_tensor->data->data[b_offset];

                // d(a^b)/da = b * a^(b-1)
                if (a->requires_grad) {
                    float grad_a_val = 0.0f;
                    if (!(aval == 0.0f && (bval - 1.0f) < 0.0f)) {
                        grad_a_val = bval * powf(aval, bval - 1.0f);
                    }
                    a->grad->data[a_offset] += dout * grad_a_val;
                }

                // d(a^b)/db = a^b * log(a)
                if (b_tensor->requires_grad) {
                    float grad_b_val = 0.0f;
                    if (aval > 0.0f) { // log is only defined for positive numbers
                        grad_b_val = powf(aval, bval) * logf(aval);
                    }
                    b_tensor->grad->data[b_offset] += dout * grad_b_val;
                }
            }
        } else {
            ...
            for (; i + SIMD_WIDTH - 1 < size; i += SIMD_WIDTH) {
                ...
                if (b_tensor->requires_grad) {
                    ...
                    log_x = _mm256_blendv_ps(log_x, zero, non_positive_x_mask); // Set log(x) to 0 if x <= 0
...
void div_grad_op(Tensor* out, Tensor** prev, int n_prev, void* extras) {
    ...
    if (!out->grad->data) {
        LOG_ERROR("div_grad_op ERROR: Output gradient is NULL! out->grad=%p", (void*)out->grad->data);
        return;
    }
...
void matmul_grad_op(Tensor* out, Tensor** prev, int n_prev, void* extras) {
    ...
    if (!out->grad->data) {
        LOG_ERROR("matmul_grad_op ERROR: Output gradient is NULL! out->grad=%p", (void*)out->grad->data);
        return;
    }
        ...
        for (int batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
            ...
            for (int i = 0; i < N; ++i) {
                for (int j = 0; j < K; ++j) {
                    float sum = 0.0f;
                    for (int k = 0; k < M; ++k) {
                        float out_grad_val = out->grad->data[out_batch_offset + i * out_row_stride + k * out_col_stride];
            ...
            for (int i = 0; i < K; ++i) {
                for (int j = 0; j < M; ++j) {
                    ...
                    for (int k = 0; k < N; ++k) {
                        ...
                        float out_grad_val = out->grad->data[out_batch_offset + k * out_row_stride + j * out_col_stride];
                                ...
                                for (int ow = ow_start; ow < ow_end; ++ow) {
                                    ...
                                    if (ih >= 0 && ih < Hin && iw >= 0 && iw < Win) {
                                        for (int cout = 0; cout < Cout; ++cout) {
                                            float out_grad_val = out->grad->data[n * Cout * Hout * Wout + cout * Hout * Wout + oh * Wout + ow];

                                            for (int cin = 0; cin < Cin; ++cin) {
                                                int in_idx = n * Cin * Hin * Win + cin * Hin * Win + ih * Win + iw;
                                                int kernel_grad_idx = cout * Cin * Kh * Kw + cin * Kh * Kw + kh * Kw + kw;

                                                kernel->grad->data[kernel_grad_idx] += in->data->data[in_idx] * out_grad_val;
                                            }
                                        }
                                    }
                                }
                                    ...
                                    for (int ow = ow_start; ow < ow_end; ++ow) {
                                        ...
                                        if (ih >= 0 && ih < Hin && iw >= 0 && iw < Win) {
                                            ...
                                            for (int cin = 0; cin < Cin; ++cin) {
                                                ...
                                                in->grad->data[in_grad_idx] += kernel->data->data[kernel_idx] * out_grad_val;
                                            }
                                        }
                                    }
                                }
