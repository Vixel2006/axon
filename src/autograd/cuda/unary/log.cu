...
__global__ void noncontig_log_grad_kernel(const float* out_grad, const float* prev_data,
                                          float* prev_grad, int n, const int* prev_shape,
                                          ...
                                          const int* out_shape, const int* out_strides, int out_ndim)
{
    ...
    for (int i = idx; i < n; i += stride)
    {
        ...
        atomicAdd(&prev_grad[in_idx], out_grad[i] / (prev_data[in_idx] + 1e-7f));
    }
}
...
void log_grad_op_cuda(Tensor* out, Tensor** prev, int n_prev, void* extras)
{
    ...
    if (prev[0]->requires_grad)
    {
        ...
        else
        {
            ...
            noncontig_log_grad_kernel<<<num_blocks, num_threads_per_block>>>(
                out->grad->data->data, prev[0]->data->data, prev[0]->grad->data->data, N,
