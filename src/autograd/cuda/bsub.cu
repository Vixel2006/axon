...
__global__ void bsub_kernel(const float* out_grad_p,
                            float* a_grad_p,
                            float* b_grad_p,
                            bool a_req_grad,
                            bool b_req_grad,
...
void CudaAutograd::sub(Tensor& out, std::vector<Tensor>& prev) {
    ...
    if (!out_grad_p || (!a_req_grad && !a_grad_p) || (!b_req_grad && !b_grad_p)) {
         throw std::runtime_error("A gradient pointer is null in 'sub' backward pass (CUDA).");
    }

    const int threadsPerBlock = 256;
    const int blocksPerGrid = (num_elements + threadsPerBlock - 1) / threadsPerBlock;

    bsub_kernel<<<blocksPerGrid, threadsPerBlock>>>(
        out_grad_p,
        a_grad_p,
        b_grad_p,
        a_req_grad,
