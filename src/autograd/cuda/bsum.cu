    ...
    size_t num_elements)
{
    ...
    for (size_t i = index; i < num_elements; i += stride) {
        ...
        grad_a_data[i] += grad_out_data[out_idx];
    }
}
...
void CudaAutograd::sum(Tensor& out, std::vector<Tensor>& prev) {
    ...
    if (!out_grad_p || !a_grad_p) {
        throw std::runtime_error("A gradient pointer is null in 'sum' backward pass (CUDA).");
    }
    ...
    sum_backward_kernel<<<blocksPerGrid, threadsPerBlock>>>(
        out_grad_p,
        a_grad_p,
        d_in_strides,
        d_out_strides,
