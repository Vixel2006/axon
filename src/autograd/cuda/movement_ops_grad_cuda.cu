...
{
    LOG_INFO("concat_grad_op_cuda: Entering function with n_prev=%d", n_prev);
    ...
    assert(n_prev > 0 && "Number of previous tensors must be greater than 0");
    assert(extras && "Extras (axis) cannot be NULL");

    int axis = *(int*) extras;
    assert(axis >= 0 && axis < out->ndim && "Axis is out of bounds");

    if (out->grad == NULL || out->grad->data == NULL)
    {
        LOG_WARN("Output tensor has no gradient data, skipping backward pass for concat.");
        assert(0 && "Output tensor has no gradient data, skipping backward pass for concat.");
    }
    ...
    for (int i = 0; i < n_prev; ++i)
    {
        ...
        assert(current_prev && "Current previous tensor cannot be NULL");
        assert(current_prev->data && "Current previous tensor data cannot be NULL");
        assert(current_prev->data->data && "Current previous tensor data pointer cannot be NULL");
        assert(current_prev->shape && "Current previous tensor shape cannot be NULL");
        assert(current_prev->strides && "Current previous tensor strides cannot be NULL");

        if (current_prev->requires_grad)
        {
            assert(current_prev->grad &&
                   "Current previous tensor gradient cannot be NULL if requires_grad");
            assert(current_prev->grad->data &&
                   "Current previous tensor gradient data cannot be NULL if requires_grad");
            assert(current_prev->grad->data->data &&
                   "Current previous tensor gradient data pointer cannot be NULL if requires_grad");
            ...
            else
            {
                ...
                if (err != cudaSuccess)
                {
                    ...
                    assert(0 && "Failed to allocate device memory for strides");
                }
                ...
                if (err != cudaSuccess)
                {
                    ...
                    offset_in_axis += current_prev->shape[axis];
                    assert(0 && "Failed to allocate device memory for shape");
                }
