...
__global__ void zero_grad_kernel(float** params, float** grad, const size_t* sizes, int num_tensors) {
  int t = blockIdx.y;
...
void CudaOptimizers::zero_grad(std::vector<std::shared_ptr<Tensor>>& params) {
  ...
  zero_grad_kernel<<<blocks, threads>>>(d_data_ptrs, d_grad_ptrs, d_sizes, num_tensors);
