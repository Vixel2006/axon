...
void unsqueeze_op(Tensor* in, Tensor* out, int dim) {
    ...
    if (dim < out->ndim - 1) {
        out->strides[dim] = out->strides[dim + 1] * out->shape[dim + 1];
    } else {
        out->strides[dim] = 1;
    }
...
void concat_op(Tensor** in, Tensor* out, int num_tensors, int axis) {
    ...
    for (int i = 1; i < num_tensors; ++i) {
        if (in[i]->ndim != ndim) {
            LOG_ERROR("concat_op: Input tensors must have the same ndim (%d != %d).", in[i]->ndim, ndim);
            return;
        }
        for (int d = 0; d < ndim; ++d) {
            if (d != axis && in[i]->shape[d] != in[0]->shape[d]) {
                LOG_ERROR("concat_op: Input shapes mismatch on dim %d (%d != %d).", d, in[i]->shape[d], in[0]->shape[d]);
                return;
            }
        }
    }
    ...
    if (out->shape[axis] != sum_axis_size) {
        LOG_ERROR("concat_op: Output shape on axis %d mismatch (expected %d, got %d).", axis, sum_axis_size, out->shape[axis]);
        free(cum_sizes);
        return;
    }
    for (int d = 0; d < ndim; ++d) {
        if (d != axis && out->shape[d] != in[0]->shape[d]) {
            LOG_ERROR("concat_op: Output shape mismatch on dim %d (%d != %d).", d, out->shape[d], in[0]->shape[d]);
            free(cum_sizes);
            return;
        }
    }
    ...
    for (int t = 0; t < num_tensors; ++t) {
        ...
        int* dummy_str = src->strides; // Dummy, since off_b is unused
        for (int linear_idx = 0; linear_idx < in_size; ++linear_idx) {
            COMPUTE_OFFSETS(linear_idx, src, src->strides, dummy_str, out->strides, off_in, off_b, off_out);
