...
extern "C" void add_op_cuda(Tensor* a, Tensor* b, Tensor* out)
{
    LOG_INFO("add_op_cuda: Entering function");
    ...
    if (!out->data)
    {
        ...
        assert(0 && "Failed to allocate Storage for out tensor in add_op_cuda");
    }
    out->data->counter = 1;
    out->data->size = N;
