...
__global__ void noncontig_sub_kernel(const float* a, const float* b, float* out, const int n,
...
__global__ void contig_sub_kernel(const float* a, const float* b, float* out, const int n)
{
...
extern "C" void sub_op_cuda(Tensor* a, Tensor* b, Tensor* out)
{
    LOG_INFO("sub_op_cuda: Entering function");
    ...
    if (!out->data)
    {
        ...
        assert(0 && "Failed to allocate Storage for out tensor in sub_op_cuda");
    }
    out->data->counter = 1;
    out->data->size = N;
    ...
    if (err != cudaSuccess)
    {
        ...
        assert(0 && "Failed to allocate CUDA memory for out->data->data in sub_op_cuda");
    }

    if (is_contiguous(a) && is_contiguous(b) && is_contiguous(out))
    {
        contig_sub_kernel<<<num_blocks, num_threads_per_block>>>(a->data->data, b->data->data,
                                                                 out->data->data, N);
    }
    else
    {
        noncontig_sub_kernel<<<num_blocks, num_threads_per_block>>>(
