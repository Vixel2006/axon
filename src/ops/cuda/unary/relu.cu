...
__global__ void contig_relu_kernel(const float* a, float* b, int n)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
...
extern "C" void relu_op_cuda(Tensor* in, Tensor* out)
{
    LOG_INFO("relu_op_cuda: Entering function");
    ...
    if (!out->data)
    {
        ...
        assert(0 && "Failed to allocate Storage for out tensor in relu_op_cuda");
    }
    out->data->counter = 1;
    out->data->size = N;
