...
__global__ void noncontig_neg_kernel(const float* a, float* b, int n, const int* a_shape,
...
__global__ void contig_neg_kernel(const float* a, float* b, int n)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
...
extern "C" void neg_op_cuda(Tensor* in, Tensor* out)
{
    LOG_INFO("neg_op_cuda: Entering function");
    ...
    if (!out->data)
    {
        ...
        assert(0 && "Failed to allocate Storage for out tensor in neg_op_cuda");
    }
    out->data->counter = 1;
    out->data->size = N;
    ...
    if (err != cudaSuccess)
    {
        ...
        assert(0 && "Failed to allocate CUDA memory for out->data->data in neg_op_cuda");
    }

    if (is_contiguous(in) && is_contiguous(out))
    {
        contig_neg_kernel<<<num_blocks, num_threads_per_block>>>(in->data->data, out->data->data,
                                                                 N);
    }
    else
    {
        noncontig_neg_kernel<<<num_blocks, num_threads_per_block>>>(in->data->data, out->data->data,
