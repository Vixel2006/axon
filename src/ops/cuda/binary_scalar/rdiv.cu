...
__global__ void contig_rdiv_scalar_kernel(const float* in, float* out, float scalar, int n)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;
...
{
    LOG_INFO("rdiv_scalar_op_cuda: Entering function");
    // Check for division by zero in the input tensor elements
    // This check is more complex as it would require a kernel launch to check all elements.
    // For now, we'll assume the user handles this or that CUDA will produce Inf/NaN.
    ...
    {
        contig_rdiv_scalar_kernel<<<num_blocks, num_threads_per_block>>>(a->data->data, out->data->data, b, N);
    }
    else
    {
        noncontig_rdiv_scalar_kernel<<<num_blocks, num_threads_per_block>>>(
    ...
    LOG_INFO("rdiv_scalar_op_cuda: done successfully");
}
