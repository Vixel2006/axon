...
__global__ void contig_div_scalar_kernel(const float* in, float* out, float scalar, int n)
{
    int idx = blockDim.x * blockIdx.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;
...
{
    LOG_INFO("div_scalar_op_cuda: Entering function");
    if (b == 0.0f)
    {
    ...
    {
        contig_div_scalar_kernel<<<num_blocks, num_threads_per_block>>>(a->data->data, out->data->data, b, N);
    }
    else
    {
        noncontig_div_scalar_kernel<<<num_blocks, num_threads_per_block>>>(
    ...
    LOG_INFO("div_scalar_op_cuda: done successfully");
}
