...
class Sequential:
    ...
    def add(self, name, module):
        ...
        if not isinstance(module, dict) or "fn" not in module:
            raise TypeError(
                f"Module for '{name}' is not a valid dictionary created by a @nw.pipe function."
            )
    ...
    def summary(self, input_shape):
        ...
        summary_str = (
            f"Sequential (Input: {list(input_shape)})\n{separator}\n{header}\n"
        )
        ...
        for name, module in self.modules.items():
            ...
            has_trainable_params = any(
                p.requires_grad for p in module.get("params", {}).values()
            )
    ...
    def load_state_dict(self, state_dict):
        ...
        for name, loaded_data in state_dict.items():
            if name not in current_model_tensors:
                print(
                    f"Warning: Key '{name}' in state_dict is not found in the current model. Skipping."
                )
                continue
            ...
            if model_tensor.shape != loaded_data.shape:
                raise ValueError(
                    f"Shape mismatch for '{name}': loaded tensor has shape {loaded_data.shape}, but model tensor has shape {model_tensor.shape}."
                )
        ...
        for name in current_model_tensors:
            if name not in state_dict:
                print(
                    f"Warning: Key '{name}' in the current model is not found in the loaded state_dict."
                )
